...
class SarcasmDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        # First, Retrieve text and label from self.data[index]
        example = self.data[index]
        text = example["headline"]
        label = example["is_sarcastic"]

        # Use self.tokenizer to tokenize the text and
        # ensure you handle padding, truncation
        ...
        # The tokenizer outputs 2D tensors of shape (1, max_length)
        # -> .flatten() to convert them to 1D tensors of shape (max_length,)
        return {
...
class SarcasmBERT(nn.Module):
    def __init__(self):
        ...
        # bert-base-uncased outputs vectors of size 768
        # -> map this to 2 classes (0 or 1).
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # extract the standard outputs from the BERT model
        ...
        # The [CLS] token is always the first token in the sequence (index 0)
    ...
    **kwargs
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            # forward pass, loss calculation, backward pass (to compute gradients)
